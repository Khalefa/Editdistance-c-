...
namespace RTree
{
    ...
    public class RTree<T>
    {
        ...
        private delegate void pairproc(Object x, Object y);
        /// <summary>
        ...
        private void joins(RTree<T> r, Node<T> m, Node<T> n, int dist)
        {
                ...
                for (int j = 0; j < m.entryCount; j++)
                {
                    ...
                    if (mj.intersects(ri))
                    {
                            ...
                            if (!n.isLeaf() && !m.isLeaf())
                            {
                                ...
                                Node<T> cNode = r.getNode(m.ids[j]);
                                if (childNode.isLeaf() && cNode.isLeaf())
                                    joined.Add(new pair(childNode, cNode));
                                else
                                joins(r,cNode,  childNode, dist);
                            }else{
        ...
        {
             joined = new List<pair>();
            Node<T> rootNode = getNode(rootNodeId);
            Node<T> m = r.getNode(r.rootNodeId);
            joins(r, m, rootNode, distance);
            return joined;
        }
